...
getExpiredJobsByProfession = function(prof,successCB,errorCB)
{
	try
	{
		db.cypher({
			query:'match(j:Job)-[r]->(p:Profession) where p.name in {profession} and toInt(j.closedate) < toInt(timestamp()) return j.name as name',
			params:{
				profession:prof
			}
		},
...
getAvailableJobsByProfession = function(prof,successCB,errorCB)
{
	try
	{
		db.cypher({
...
module.exports = {
	...
	getExpiredJobsByProfession: getExpiredJobsByProfession,
	getAvailableJobsByProfession: getAvailableJobsByProfession,
	getIntent: getIntent
};
